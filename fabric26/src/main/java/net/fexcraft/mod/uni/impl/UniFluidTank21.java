...
public class UniFluidTank21 extends UniFluidTank {
	...
	public void amount(String type, int am){
		clear();
		FluidVariant var = FluidVariant.of(BuiltInRegistries.FLUID.getValue(Identifier.parse(type)));
		if(var == null) return;
		storage.variant = var;
		storage.amount = am;
	}
	...
	public String getFluid(){
		return storage.variant.typeHolder().getRegisteredName();
	}
	...
	public String getFluidFromStack(StackWrapper sw){
		...
		for(StorageView<FluidVariant> var : stor){
			return var.getResource().typeHolder().getRegisteredName();
		}
		return null;
	}
