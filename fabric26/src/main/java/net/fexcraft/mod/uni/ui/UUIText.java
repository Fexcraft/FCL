...
public class UUIText extends UIText {
	...
	public void draw(Object gui, UIElement root, float ticks, int gl, int gt, int mx, int my){
		if(!visible) return;
		net.fexcraft.mod.uni.ui.UniUI uui = (UniUI)gui;
		...
		if(scale == 0.0F || (scale < 0.0F && textwidth < width)){
			uui.matrix.text(uui.getMinecraft().font, value, xx, yy, UniUI.convCol(hovered ? hover : color), shadow);
		}
		else{
			...
			uui.matrix.text(uui.getMinecraft().font, value, 0, 0, UniUI.convCol(hovered ? hover : color), shadow);
