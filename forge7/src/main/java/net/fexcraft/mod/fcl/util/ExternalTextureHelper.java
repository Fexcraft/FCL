	...
	public static class SimplerThreadImageDownloader extends SimpleTexture {
	    ...
	    public void loadTexture(IResourceManager resman) throws IOException {
			...
			thread = new Thread(() -> {
				...
				FCL.LOGGER.info("Attempting download of texture '" + url + "'.");
				try{
					...
					if(conn.getResponseCode() == 200){
						...
						FCL.LOGGER.info("Received response code '" + conn.getResponseCode() + "'.");
					}
				}
				catch(Exception e) {
					FCL.LOGGER.info("Errors during download of texture.");
					e.printStackTrace();
					return;
				}
				finally{
