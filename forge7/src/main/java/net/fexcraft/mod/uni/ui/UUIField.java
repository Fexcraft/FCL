...
public class UUIField extends UIField {
	...
	public UUIField(UserInterface ui, JsonMap map) throws Exception {
		super(ui, map);
		field = new GuiTextField(Minecraft.getMinecraft().fontRenderer, 0, 0, width, height){
			@Override
	...
	public void draw(Object gui, UIElement root, float ticks, int gl, int gt, int mx, int my){
		if(!visible()) return;
		field.xPosition = absolute ? x < 0 ? ui.screen_width + x : x : gl + x;
		field.yPosition = absolute ? y < 0 ? ui.screen_height + y : y : gt + y;
		field.drawTextBox();
	}
