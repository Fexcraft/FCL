	...
	protected PolygonShape compileShape(){
    	...
    	for(int j = 1; j < rings; j++){
    		for(int i = 0; i < segments; i++){
    			...
    			float xSize = MathHelper.sin((FexcraftModelRenderer.PI / segments) * i * 2F + FexcraftModelRenderer.PI) * yWidth;
    			float zSize = -MathHelper.cos((FexcraftModelRenderer.PI / segments) * i * 2F + FexcraftModelRenderer.PI) * yWidth;
