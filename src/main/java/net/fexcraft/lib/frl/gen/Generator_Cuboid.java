...
public class Generator_Cuboid {
	...
	public static <GLO> void make(Polyhedron<GLO> poly, ValueMap map){
		...
		if(shaped){
			ArrayList<Vec3f> array = map.getArray("corners", 8);
	...
	private static Polygon genPolygonWithUV(int index, float[] uv, boolean detached, float w, float h, Vertex[] vertices, float tu, float tv, float x, float y, float ex, float ey){
		...
		if(cuv == null){
			...
			float ys = tv + y;
			float ye = tv + y + ey;
			vertices[0] = vertices[0].uv(xe / w, ys / h);
	    	vertices[1] = vertices[1].uv(xs / w, ys / h);
	    	vertices[2] = vertices[2].uv(xs / w, ye / h);
	    	vertices[3] = vertices[3].uv(xe / w, ye / h);
		}
			...
			vertices[0] = vertices[0].uv((tu + cuv[0]) / w, (tv + cuv[1]) / h);
	    	vertices[1] = vertices[1].uv((tu + cuv[2]) / w, (tv + cuv[3]) / h);
	    	vertices[2] = vertices[2].uv((tu + cuv[4]) / w, (tv + cuv[5]) / h);
	    	vertices[3] = vertices[3].uv((tu + cuv[6]) / w, (tv + cuv[7]) / h);
		}
		else {}
