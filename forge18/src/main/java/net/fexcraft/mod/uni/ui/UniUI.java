...
public class UniUI extends AbstractContainerScreen<UniCon> {
	...
	public UniUI(UniCon con, Inventory inventory, Component component){
		...
		ui.drawer = new UserInterface.Drawer() {
			...
			public void draw(float x, float y, int u, int v, int w, int h){
				bind(actex);
				blit(matrix, (int)x, (int)y, u, v, w, h);
			}

			@Override
			public void drawFull(float x, float y, int w, int h){
				bind(actex);
				blit(matrix, (int)x, (int)y, 0, 0, w, h, w, h);
			}

			@Override
			public void draw(int x, int y, StackWrapper stack, boolean text){
				//TODO matrix.renderItem(stack.local(), x, y);
				//TODO if(text) matrix.renderItemDecorations(font, stack.local(), x, y);
			}
	...
	@Override
	protected void renderBg(PoseStack matrix, float ticks, int mx, int my){
		this.matrix = matrix;
		...
		if(tooltip.size() > 0){
			comtip.clear();
			for(String str : tooltip) comtip.add(new TextComponent(str));
			renderTooltip(matrix, comtip, Optional.empty(), mx, my);
		}
		Slot slot = getSlotUnderMouse();
		if(slot != null && !slot.getItem().isEmpty()){
			List<Component> list = slot.getItem().getTooltipLines(menu.player, TooltipFlag.Default.ADVANCED);
	...
	public void drawbackground(float ticks, int mx, int my){
		for(UITab tab : ui.tabs.values()){
			...
			bindTexture(tab.texture);
			if(tab.absolute){
				blit(matrix, (tab.x < 0) ? (width + tab.x) : tab.x, (tab.y < 0) ? (height + tab.y) : tab.y, tx, ty, tab.width, tab.height);
				continue;
			}
			blit(matrix, leftPos + tab.x, topPos + tab.y, tx, ty, tab.width, tab.height);
		}
