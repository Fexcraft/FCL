...
public class UUIText extends UIText {
	...
	public void draw(Object gui, UIElement root, float ticks, int gl, int gt, int mx, int my){
		...
		if(scale == 0.0F || (scale < 0.0F && textwidth < width)){
			uui.drawString(uui.matrix, uui.getMinecraft().font, value, xx, yy, hovered ? hover.packed : color.packed);//, shadow);
		}
		else{
			float scale = this.scale < 0.0F ? width / (float)textwidth : this.scale;
			uui.matrix.pushPose();
			uui.matrix.translate(xx, yy, 0.0F);
			uui.matrix.scale(scale, scale, scale);
			uui.drawString(uui.matrix, uui.getMinecraft().font, value, 0, 0, hovered ? hover.packed : color.packed);//, shadow);
			uui.matrix.popPose();
		}
	}
